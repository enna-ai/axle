class Player {
  constructor() {
    this.pos = createVector(19 * tileSize + xoff, 8 * tileSize + yoff);
    ...
    this.human = false;
    this.coins = [
      new Coin(3.5 * tileSize + xoff, 8 * tileSize + yoff),
      new Coin(10.5 * tileSize + xoff, 3.5 * tileSize + yoff),
      new Coin(16 * tileSize + xoff, 5 * tileSize + yoff),
      new Coin(8 * tileSize + xoff, 2.5 * tileSize + yoff),
    ];
    this.setNodes();
  };
  ...
  setNodes() {
    ...
    this.nodes[4] = new Node(tiles[19][2], true, false);
  ...
  show() {
    fill(253, 253, 255, this.fadeCounter);
    ...
    stroke(0);
    for (let coinNum = 0; coinNum < this.coins.length; coinNum++) {
  ...
  move() {
    ...
    let temp = createVector(this.vel.x, this.vel.y);
    temp.normalize();
